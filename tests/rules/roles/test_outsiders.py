...
def test_butler_acts_on_both_night_types_and_can_choose_a_dead_master():
    ...
    assert Butler().first_night_order is not None
    assert Butler().other_night_order is not None
    assert Butler().apply(context, AbilityChoice("alice", ("bob",))) == [
        RuleEffect("set_master", {"butler_id": "alice", "master_id": "bob", "requires_healthy": True})
    ]
    assert Butler().may_vote(context, master_is_voting=True)
    assert not Butler().may_vote(context, master_is_voting=False)


def test_dead_or_misinformed_butler_defers_vote_validity_to_the_normal_vote_rules():
    game = game_with_roles(alice="butler", bob="chef", carol="imp")
    game.players["alice"].alive = False

    assert Butler().may_vote(AbilityContext.from_state(game, "alice"), master_is_voting=False) is None
    game.players["alice"].alive = True
